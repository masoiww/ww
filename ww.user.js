...
let PLAYERS = []
let ROLE = undefined
let GAME_STATUS = undefined
let IS_CONSOLE_OPEN = false
let GOLD_WHEEL_SPINS_COUNTER = 0
let GOLD_WHEEL_SILVER_SESSION = 0
let TOTAL_XP_SESSION = 0
let TOTAL_UP_LEVEL = 0
let GAME_STARTED_AT = 0
let DOCUMENT_TITLE = undefined
let LV_SETTINGS = {
  DEBUG_MODE: false,
...
let AUTO_REPLAY_INTERVAL = undefined
let SOCKET = undefined
let REGULARSOCKET = undefined
...
const connectSocket = () => {
  let LOVERS = []
  let DEADS = []
  let JW_TARGET = undefined
  let CHAT_WW_SENDED = false
  let WOLVES = []
  let TARGET_WW_VOTE = undefined
  const url = `wss://${SERVER_URL.replace('https://', '')}/`
  SOCKET = io(url, {
    query: {
      firebaseToken: AUTHTOKENS.idToken,
...
function scrollToBottom() {
  let elems = document.getElementsByClassName('lv-chat-container')
