...
async function chat_me() {
  ...
  // Define the XPath for the textarea element.
  const xpath = '//*[@id="root"]/div/div/div/div/div[1]/div/div/div/div/div/div/div/div/div/div/div/div/div[1]/div[1]/div[2]/div[1]/div[3]/div/div/div/div[2]/div/textarea';
  
  // Use document.evaluate to get the textarea.
  const textareaResult = document.evaluate(
    xpath,
    document,
    null,
  ...
  const textarea = textareaResult.singleNodeValue;
...
function me_wc() {
  ...
  // Declare extractedNumber outside the loop so it's accessible later.
  let extractedNumber = NaN;

  // Loop over the divs to find the first matching element with a valid number.
  for (const div of divs) {
    // Check if any of the search terms exist in the div's text content.
    if (searchTerms.some(term => div.textContent.includes(term))) {
      const span = div.querySelector('span.css-le9x18');
      if (span) {
        extractedNumber = parseInt(span.textContent, 10);
        // Only break out of the loop if the extracted number is valid (not NaN)
  ...
  if (myRole && myRole.includes("wolf") && chatSun && !isNaN(extractedNumber)) {
    ...
    BotVote(row_index, column_index);
  }
}
